...
do <- function(n, cen) {
    ...
    tt <- seq(0, 1.5, .01)
    ## Fitting
    fit <- rocTree(Surv(Y, death) ~ z1 + z2, data = dat, id = id, control = list(CV = TRUE))
    fit.dcon <- rocTree(Surv(Y, death) ~ z1 + z2, data = dat, id = id, control = list(splitBy = "dCON", CV = TRUE))
...
tmp <- parSapply(NULL, 1:100, function(z) do(200, 0))
stopCluster(cl)
