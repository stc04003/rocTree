...
* `prune` a logical vector specifying whether to prune the survival tree. If `TRUE`, a cross-validation procedure will be performed to determine the optimal sub-tree; the default value is `TRUE`.
...
* `Frame` is a data frame describe the resulting tree. The columns are 
  ...
  * `u2` is the minimum proportion in $\tau_L$ across time.
...
plot(fit, rankdir = "LR", shape = "rect")
