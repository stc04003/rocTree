...
Random forests improve the variance reduction of bagging
by reducing the correlation between the trees via random selection of predictors in the tree-
growing process.
In the following, we grow the random forest with fully grown trees with small terminal nodes and without pruning.
...
The function `rocForest` returns an object of S3 class -- `rocForest`. 
The 500 survival trees are stored in `fit$forest`.
