...
\newtheorem{remark}{Remark}
  ...
  \newcommand{\blue}[1]{#1} % turn off for latexdiff
...
\noindent{\blue{We thank the Editor, the Associate Editor and three referees for their very thorough reviews and constructive comments. We have undertaken a complete and exhaustive revision of the manuscript in response to the reviews. 
%In this revision, we have carefully revised the manuscript to address the comments in the review. 
Several paragraphs in the manuscripts have been rewritten for improved readability, and more simulations and analysis have been conducted. Below please find our point-to-point response to reviewers' comments.}}
...
In the following, we address each of your
concerns in turn:
\begin{enumerate}
	\item We highlight the important features that make our approaches more feasible than some of our competitors. 
	%the usually estimation procedure based on a U-statistic. 
At each time $t$, the concordance probability of a partition-based risk score can be decomposed into sums of node-level estimates (Equation (\cy{7})), thus the computational cost is much lower than the usual estimation procedure based on a U-statistic. The computational cost also depends on the number of time points considered in the global measure and, through our exploration in simulation studies, a moderate number of time points gives a reasonably good performance. A discussion on
the number of time points is added on page \cy{12}. 
Moreover, in our second splitting rule where the splitting based on the increment of the integrated concordance measure (page \cy{14}), the calculation is based on a weighted average of the absolute difference between hazards of two child nodes to further reduce the computational burden. 
%does not require intensive computation.
	\item The kernel type estimators can result in biased estimation when the terminal nodes contain a small number of observations. To avoid the potential biased estimation, we control the node sizes by a pruning procedure as well as a stopping rule that sets the minimum node size. With these settings, a large tree with small node sizes is not very likely to be selected as the final model. We have expanded the discussion on pruning in Section 3. In practice, if the true model is complex and a very large tree is needed to fit the data, we recommend our random forest procedure for stable prediction. A discussion is added on page \cy{25}. 
	...
	For estimation of a tree with a finite number of terminal nodes, our discussion in Section 2 and 3 is under stronger censoring assumptions to ensure the validity of the node-specific estimates. We note that this problem also exists in previous works of survival trees. As a solution, our methods can be extended to handle conditionally independent censoring by modeling the conditional censoring distribution \citep{steingrimsson2018censoring}. A detailed discussion is given in the Supplementary Materials. %Due to space limit, the details are given in Supplementary Materials.
...
\noindent{\blue{\textbf{Response}: In this revision, we have revised the simulation studies and included the comparison with a Cox model with time-dependent covariates.}}\red{I will add some discussion to this response when I have the Brier scores.}
...
\noindent{\blue{\textbf{Response}: We have removed the independence symbol and explicitly state the independence assumption: ``{For ease of discussion, we assume  $C$ is independent of $\{ T, Z(\cdot)\}$ in Section 3.2''.}}}

\emph{ - Second sentence below proposition 2 starts in an unclear way.
}

\noindent{\blue{\textbf{Response}: We have revised the sentence to ``When \blue{$\widehat{\omega}(\cdot)>0$}, the equality holds if and only if $\widehat{\lambda}(\cdot\mid\tau_1^L) = \widehat{\lambda}(\cdot\mid\tau_1^R)$ almost everywhere on $(0,s]$''.}}

\emph{ -  The considered independent censoring assumptions seems strong (i.e. stronger than what I believe is typically needed).
}

\noindent{\blue{\textbf{Response}: The conditionally independent censoring assumption (A1 on page7) is required for the large-sample result of the survival tree to hold and is consistent with the literature. However, the consistency results of the survival trees do not account for the data dependent splitting and pruning algorithm. Thus, a valid estimation of the splitting criteria (e.g., log-rank statistic and the proposed concordance measure) typically requires a strong censoring assumption. We have followed the guidance of the Associate Editor and moved the necessary conditions for Theorem 1 to the main context to improve readability. We also provided an extension to handle the milder conditionally independent censoring assumptions in splitting in the Supplementary Materials.}}



\noindent \textbf{Response to Associate Editor's comments:}

\emph{The authors propose a method for constructing survival trees and forests with time-dependent covariates. I think this work is quite interesting, but could be improved in several respects. My own comments follow, which emphasize the important points raised by the referees; in addition to this, the authors should carefully attend to the particular comments of Referee 3 in revising their manuscript.} \red{I think we can remove this paragraph too?}
...
\noindent{\blue{\textbf{Response}: 
We appreciate the recognition of the important contributions of our work. We have revised the manuscript throughout to reiterate each of these contributions. In particular, we have expanded the introduction (page \cy{4}) to emphasize the use of integral of time-dependent AUC and time-invariant partition. 
We also adjusted the presentation of the manuscript by first introducing the time-invariant partition and the estimation of partition-based risk function in Section 2, and moving all the materials related to how to build such a survival tree using ROC to Section 3.}}
...
\noindent{\blue{\textbf{Response}: 
To better illustrate the interpretability of the proposed method, we have added a new paragraph on page \cy{5--6} and a new plot (Figure 1) to demonstrate how to interpret the time-invariant survival tree in the case of a single predictor.}\\
%To address your comments, we have added a discussion of how to interpret the results on page \cy{5--6}; we also added an example for better illustration.}\\
\begin{quote}
...
To conserve computational recourse, one
%If researchers have limited computational resources, they 
can start from a less refined grid or fit the tree on a shorter time interval. A discussion is now added on page 12.}}
...
\noindent{\blue{\textbf{Response}: To address your comment, we have moved the survival tree and forest algorithms into the text as Algorithm 1 and 2. More details are added to clear up potential misconceptions.}}
...
\noindent{\blue{\textbf{Response}: %Thanks for the comments. In this revision, we have added a discussion of bandwidth selection. 
For one-sample nonparametric kernel smoothing estimator, the optimal global bandwidth on the time interval depends on the sample size and underlying distribution, thus we use different bandwidths for different nodes in the survival tree algorithm. On page 19-20, we have added the following remark.}}
...
\noindent{\blue{%Note that 
When using forest for hazard prediction, the bandwidth in (11) also needs to be selected. We have added the following discussion.
\begin{quote}
...
\noindent{\blue{\textbf{Response}: Your understanding is correct. 
We denoted these two subsets as ${\mathcal I}_{1b}$ and ${\mathcal I}_{2b}$, where the former is only used to build the survival tree and the latter is only used to calculate the weight. 
%One subset is only used to calculate the weight and the other is only used to build the survival tree. 
We have added the following paragraph to provide the motivation for applying the sample splitting technique and the formula for weight calculation on page XX.}}
...
\end{quote}
\emph{A clearer description of what the survival forests algorithm is doing would be helpful. The manuscript says the details are in the supplementary material, but I could not find any details there. Just to give a trivial example, random forests often do not need to be pruned, and I assume the authors are not pruning when they use random forests, but I could not find this detail discussed. And, given the smoothing, is it even possible to grow out the entire tree without a disaster occurring?}

\noindent{\blue{\textbf{Response}: 
We have now moved the survival forest algorithm from the Appendix to the main text as Algorithm 2. 
As in most random forest algorithms, we do not prune when constructing random forests.
%To improve the presentation, we have moved the survival forest algorithm from the Appendix to the main text (please see Algorithm 2). For the specific question on pruning, we do not prune in our forest algorithm. 
However, unlike other survival forest algorithm that directly average the prediction from individual trees, the hazard prediction of these large trees are \underline{not} used in our final prediction/estimation. Instead, we use a weighted training sample to estimate the hazard function with weights calculated from each forest tree.
%and the weights are calculated using the partition from each tree. 
Simulation studies shows our algorithm is computationaly stable even with small terminal node sizes.}}
...
\begin{itemize}
	...
	\noindent{\blue{\textbf{Response}: 
	We have been preparing to release an R package that his hosting on GitHub under the tentative name of ``\texttt{rocTree}'' (\url{https://github.com/stc04003/rocTree}).
	The package will be made more public available by submitting to CRAN soon.}}
	...
	\noindent{\blue{\textbf{Response}:  In this revision, we have revised the simulation studies and added the comparison with the Cox model with time-dependent covariates. In particular, we have observed that the Cox model outperforms our method as well as other survival tree competitors in some scenarios. However, a simple model like the Cox model might yields accurate estimations, it does not provide a decision rule nor does it partition the covariate space into subsets.
	}}
...
\noindent{\blue{\textbf{Response}:  Your comment is very much appreciated. In this revision, we have expanded the discussion of existing tree-based methods and provided detailed discussion to some of the key references. We also made changes to the presentation of our methodology to improve readability.}}
...
We have been preparing to release an R package that his hosting on GitHub under the tentative name of ``\texttt{rocTree}'' (\url{https://github.com/stc04003/rocTree}).
	The package will be made more public available by submitting to CRAN soon.
The CPCRA data used in our application is not publicly available, thus we include a simulated dataset in our package.}}
...
\noindent{\blue{\textbf{Response}:  (a) The integrals of $\CON_t$ on $[0,t_0]$ and $(0,t_0]$ are usually the same. Since we are interested in hazard function for $t>0$, we have followed the Reviewer's suggestion and use $(0, s]$, for some pre-specified fixed value $s$. We have added a sentence to clarify the role of $s$ on page \cy{5}: ``Let $s$ be a pre-specified constant''.}}
...
\noindent{\blue{\textbf{Response}: 
The original notation was intended to denote the quantity is estimated using the ``\textbf{u}n\textbf{c}ensored'' survival times.
We did not explain the superscript and overlook the possible misrepresentation.
We followed the Reviewer's suggestions and changed notation to $F^{\ast}$.}}
...
\noindent{\blue{\textbf{Response}: We agree that ICON is a better acronym and adopted the suggestion in the revision.}}
...
\noindent{\blue{\textbf{Response}: 
The index in the summation iterate through the partitions in $\T$. 
To avoid confusion and to express $\CON$ with a common denominator, we have rewritten the expression of $\CON_t(\cdot)$ as
%Given ${\T} = \{ \tau_1,\tau_2,\ldots,\tau_M \}$, we have
\begin{align}
\nonumber
...
\label{con}
\end{align}
for ${\T} = \{ \tau_1,\tau_2,\ldots,\tau_M \}$.}}
\noindent{\blue{To show equation~\eqref{con} holds, we note the relationship
\begin{align*}
...
\end{align*}
Thus the equation~\eqref{con} is proved. \red{Should we put this in the supplementary, since he asked?}}}
...
notation from Section 1.1. of the Supplementary Material?  I have a
bigger problem which arises when we get to $s^{\mbox{opt}}=\arg\max_s
...$; see below.}

\noindent{\blue{\textbf{Response}: 
We agree using ${\T}_s$ to denote the partition after splitting could cause confusion as the subscript ``$s$'' does not represent an index here. We thus dropped the subscript ``$s$'' and use $\T'$ to denote the partition after splitting. We did not use the notation introduced in Section 1.1 of the Supplementary Material because it is not clear which nodes are the two child nodes after splitting and the definition of $\T'$ set the stage for Proposition 2. 
With the subscript ``$s$'' removed, we address the issue with $s^{\mbox{opt}}=\arg\max_s$ separately. }}
...
\noindent{\blue{\textbf{Response}: 
We agree the usage of $s$ could cause misconception and now define optimal splits by words without introducing the $s$ notation.}} 
\begin{quote}
...
\noindent{\blue{\textbf{Response}: 
We attempted to use the zero subscript to emphasize $Z_0(t)$ is an independent observation not in the training set. 
For that reason, its subscript is out of the scope of $i$. 
We have revised the sentence to be ``Given a new observation $Z_0(t)$ {that is independent of the training sample}''.}} 
...
\noindent{\blue{\textbf{Response}: 
When the true relationships are obviously unknown, cross-validation approaches can be applied to choose the tuning parameter $m$. On the contrary, we chose $m=\lceil\sqrt{p}\rceil$ that is a typical choice in random forests \citep{friedman2001elements} and was used in many existing works of survival forests \citep{zhu2012recursively,steingrimsson2018censoring}. 
We have indicated that ``Following many existing works in survival forests \citep{zhu2012recursively,steingrimsson2018censoring}, we randomly select $\lceil \sqrt{p}\rceil$ variables at each splitting.'' }}
...
\noindent{\blue{\textbf{Response}: Thanks for pointing this out. We have replaced two simulation scenarios (VII and VIII) to include both proportional and non-proportional models in our scenarios with time-dependent covariates.
The proposed methods continue to outperform the existing methods in these new settings.}} 
...
\noindent{\blue{\textbf{Response}: We have revised the captions of the figures to make the more self-contained.}} 
...
\noindent{\blue{\textbf{Response}: As the Reviewer pointed out, $\phi_g(z)$ depends on $(c,\gamma)$. 
Since $g$, $c$, and $\gamma$ are pre-determined, we included these notations in subscripts. 
We now revise the notation to $\phi_{g,c,\gamma}(z) = I(g(z)>c) + \gamma I(g(z)=c)$ and change the proof accordingly.}}
...
\noindent{\blue{\textbf{Response}: We have followed your suggestion and revised the sentence accordingly.}} \\
...
\noindent{\blue{As an alternative, we include both random forests with and without sample-splitting in the R package.}} \\
	...
	\noindent{\blue{\textbf{Response}: We used the weight function $\omega(t) = f(t)S(t)/P(T_2 > T_1, T_1<s )$ (page 10) in both the simulations and data analysis, and the marginal distribution is estimated using the Kaplan-Meier estimator. 
	We have added the following sentence on page \cy{21} for clarification.} }
	\begin{quote}
	...
	\noindent{\blue{\textbf{Response}: We very much appreciate your comment on the theoretical developments. 
	The main focus of the current result is to justify the partition-based kernel type estimator. In the future work, we will explore the possibility of establishing large sample results of the algorithm-based estimator. }} \\
...
\begin{enumerate}
	...
	\noindent{\blue{\textbf{Response}: For survival trees, we control the node size by pruning, thus large trees whose node sizes are too small for reasonable prediction is not likely to be selected as the final tree. In this revision, we have included more details on pruning on page \cy{14}. We note that the sizes of terminal nodes generally depend on the complexity of the true hazard function $\lambda(t\mid z)$, and a small node size is not always expected. For example, if the truth is a tree with only two terminal nodes, then after pruning, the node sizes are expected to be reasonably large \red{why?}; if the truth is very complex and requires a large tree, we would recommend the use of forest-based method to stabilize the prediction. In practice, one can use cross-validation to determine the final model. We have added a discussion in Section 7.}}
	...
	\emph{(d) It is not even clear how the optimal split is chosen.  It seems it is the value which optimizes the estimated integrated concordance	index.  More discussion would be helpful.}

	\noindent{\blue{\textbf{Response}: 
	For the ICON-based splitting rule, the optimal split is chosen as \red{chosen as or chosen at?} the split that maximizes the estimated integrated concordance index. For the splitting rule based on $\Delta\ICON_\tau$, the optimal split is chosen as the split that maximizes the estimated within-node ICON. Algorithm 1 (survival tree algorithm) is added to address your comment.}} 
	...
	\noindent{\blue{\textbf{Response}: 
	When comparing to splitting criteria based on between-node heterogeneity, the ROC guided splitting criterion is appealing for its relevant to prediction accuracy because, for theoretical justification, the target hazard function is optimal and gives the highest ROC curve (Proposition 1). More importantly, the ROC guided splitting criterion is can be extended to handle time-dependent covariates. To our knowledge, approaches with Brier score loss has not been applied to the context of time-dependent covariates \red{check} and it is not clear how it can handle time-dependent covariates. The potential use of Brier score in the tree-building algorithm will be studied in our future work.}} 
	...
	\noindent{\blue{\textbf{Response}: 
	The discussion here focused on random survival forests used in practice. To avoid misconception, we have revised the sentence to be
	\begin{quote}
	...
	\end{quote}
	We agree with the reviewer that consistency results hold under assumptions regarding the terminal node size, and the bias will converge to zero as sample size goes to infinity. 
	But it would still be desirable to reduce the bias so that the convergence rate is faster. 	
	The proposed ensemble method is demonstrated to have improved prediction accuracy in simulation studies. A more rigorous theoretical justification will be a future work.}} \\
	...
	\noindent{\blue{\textbf{Response}: 
	Thank you for your careful consideration of our simulation studies.
	%We agree that when all the covariates are time-independent, it is not difficult to evaluate the performance of the Cox model.  
	Following your suggestions, we have included the Cox model for time-dependent covariates. 
	We also added Brier score and integrated Brier score to evaluate the accuracy for prediction using baseline covariates. 
	\red{I will add more on Brier scores}}} \\
